...
export const FullNameSchema = {
  ...
  validate: {
    trim: (value) => {
      ...
      return trimmedValue.length > 0 || 'Full name cannot be empty or contain only spaces';
    },
    hasTwoParts: (value) => {
      const parts = value.trim().split(/\s+/); // Split by one or more spaces
      return parts.length >= 2 || 'Full name must include at least a first name and a last name';
    },
  },
  onBlur: (e) => {
...
export const assistantNameSchema = {
  required: 'Assistant name is required',
  pattern: { value: /^[a-zA-Z\s!@#$%^&*()_+={}\[\]:;"'<>,.?/-]+$/, message: 'Invalid name' },
  validate: {
    trim: (value) => {
      const trimmedValue = value.trim();
