...
// @next
import { useLocation } from "react-router-dom"; // ✅ Use React Router
...
function NavCollapseLoop({ item, userRole, onSelect  }) {
  return item.children?.map((item) => {
    ...
    switch (item.type) {
      case 'collapse':
        return <NavCollapse key={item.id} item={item} level={1} onSelect={onSelect} />;
      case 'item':
        return (
          <Box key={item.id} sx={{ px: 0.75 }}>
            <NavItem item={item} level={1} onSelect={onSelect} />
          </Box>
        );
      default:
        return (
...
export default function NavCollapse({ item, level = 0 , onSelect }) {
  ...
  const location = useLocation(); // ✅ Use React Router for pathname
                ...
                <ClickAwayListener onClickAway={handleClose}>
                  <Box>
                    <SimpleBar sx={{ overflowX: 'hidden', overflowY: 'auto', maxHeight: '50vh' }}>
                      <NavCollapseLoop item={item} userRole={userRole} onSelect={onSelect} />
                    </SimpleBar>
                  </Box>
                </ClickAwayListener>
              </MainCard>
