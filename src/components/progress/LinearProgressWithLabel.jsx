...
export default function LinearProgressWithLabel({ placement = LinearProgressPlacement.RIGHT, showLabel = true ,value, ...props }) {
  ...
  return (
    <Stack {...stackProps} sx={{ gap: 1, ...stackProps.sx }}>
      ...
      {showLabel && <Typography variant="body2">{`${Math.round(value)}%`}</Typography>}
    </Stack>
  );
}
